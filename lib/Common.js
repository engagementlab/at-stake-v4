...
class Common extends Core {
  ...
  GetActivePlayers() {
    const players = _.pick(this._current_players, (val, key, obj) => {
      (val.connected) && ((val.decider === false) || (val.decider === undefined));
    });
  ...
  AssignRoleToPlayer(player, socket, isDecider) {
    ...
    console.log(this._active_deck_roles.length, roleIndex)
