...
class GameLogic extends Common {
  ...
  ShowScreen(screenName, data, uniqueData, refreshCurrent, socket) {
    const path = `partials/shared/${screenName}`;
    const eventId = refreshCurrent ? 'game:refresh_screen' : 'game:next_phase';
    const round = this._current_round;
    ...
    if (uniqueData) {
      ...
      function sendData(_socket) {
        ...
        // template.Load(path, outputData, (html) => {
          _socket.to(arrPlayerUnique[playerIndex].socket_id).emit(eventId, _.extend({
            name: screenName
          }, screenData, playerData));
      ...
      this.Templates.Load(`partials/shared/${screenName}`, data, (html) => {
        ...
        if (socket !== undefined) {
          socket.emit(eventId, _.extend({
            name: screenName,
            html,
          }, screenData));
        } else {
          this.groupSocket.to(this.players_id).emit(eventId, _.extend({
            name: screenName,
            html,
          }, screenData));
        }
